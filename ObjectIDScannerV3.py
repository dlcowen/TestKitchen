...
def printOBJID(file_entry, fullpath):
    for attribute in file_entry:
        if attribute.info.type == pytsk3.TSK_FS_ATTR_TYPE_NTFS_OBJID:
            ...
            wr.writerow([fullpath,object_id.rawtimestamp, object_id.timestamp, datetime.datetime.fromtimestamp(file_entry.info.meta.crtime).strftime('%Y-%m-%d %H:%M:%S'),object_id.mac,object_id.version, object_id.variant, object_id.sequence])
            return
